...
def robot_rotation(desire_x,desire_y):
    while True:
        try:
            (trans1_map,rot1_map) = listener.lookupTransform('/map', '/robot2/base_link', rospy.Time(0))
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            continue

        try:
            (trans1_odom,rot1_odom) = listener.lookupTransform('/robot2/odom', '/robot2/base_link', rospy.Time(0))
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            continue
...
if __name__ == '__main__':
    ...
    turtle_vel = rospy.Publisher('/robot2/cmd_vel_mux/input/teleop', geometry_msgs.msg.Twist, queue_size=1)
    # 3.创建 TF 订阅对象
    ...
    while not rospy.is_shutdown():
        ...
        while True:     # 根据自己mat数据的大小决定
            ...
            if i == 45:
                print("原地掉头")
                robot_rotation(42,36)
            if i == 62:
                print("原地掉头")
                robot_rotation(42,24)
            if i == 84:
                print("原地掉头")
                robot_rotation(36.5,35)
                i = i + 1
