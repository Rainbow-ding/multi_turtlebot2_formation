...
def openreadtxt(file_name):
    ...
    file.close()
    data=numpy.array(data,dtype=float)   #将其转换成numpy的数组，并定义数据类型为float
    return data
def rotate(t):
    msg.linear.x = 0.0
    msg.angular.z = 1.5 * math.pi
...
def robot_rotation(desire_x,desire_y):
    while True:
        try:
            (trans1_map,rot1_map) = listener.lookupTransform('/map', '/robot1/base_link', rospy.Time(0))
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            continue

        try:
            (trans1_odom,rot1_odom) = listener.lookupTransform('/robo1/odom', '/robot1/base_link', rospy.Time(0))
        except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
            continue
...
if __name__ == '__main__':
    ...
    turtle_vel = rospy.Publisher('/robot1/cmd_vel_mux/input/teleop', geometry_msgs.msg.Twist, queue_size=1)
    # 3.创建 TF 订阅对象

    listener = tf.TransformListener()

    rate = rospy.Rate(10.0)
    data = openreadtxt('/home/rainbow/agv1_path.txt')
    print(data)
    ...
    while not rospy.is_shutdown():
        ...
        while True:     # 根据自己mat数据的大小决定
            ...
            try:
                (trans1_odom,rot1_odom) = listener.lookupTransform('/robot1/odom', '/robot1/base_link', rospy.Time(0))
            except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
                continue
            ...
            if (Y_t - y) > 0 and (X_t - x) == 0:
                yaw_t = 0
            if (Y_t - y) > 0 and (X_t - x) < 0:
                yaw_t = math.atan((Y_t - y) / (X_t - x)) + 1.0 * math.pi
            if (Y_t - y) == 0 and (X_t - x) < 0:
                yaw_t = 0.5 * math.pi
            if (Y_t - y) < 0 and (X_t - x) < 0:
                yaw_t = math.atan((Y_t - y) / (X_t - x)) + 0.5 * math.pi
