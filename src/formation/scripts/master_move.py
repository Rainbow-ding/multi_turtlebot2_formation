...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        while i < 110:     # 根据自己mat数据的大小决定

            try:
                (trans1_map,rot1_map) = listener.lookupTransform('/map', '/robot1_tf/base_link', rospy.Time(0))
            except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
                continue

            try:
                (trans1_odom,rot1_odom) = listener.lookupTransform('/robot1_tf/odom', '/robot1_tf/base_link', rospy.Time(0))
            except (tf.LookupException, tf.ConnectivityException, tf.ExtrapolationException):
                continue
            ...
            # if (i >= 11 and i <=19) or (i >= 45 and i <= 53) or (i >= 74 and i <= 83):
            #     liner_speed = 0.2 * D_err
            #     angular_speed = 0.6 * Theta_err
            #     if liner_speed > 0.6:   # 对小车进行限制最高速度
            #         liner_speed = 0.6
            ...
            #         angular_speed = 1
            # else:
            liner_speed = 0.6 * D_err
            angular_speed = 1.5 * Theta_err
            if liner_speed > 0.8:   # 对小车进行限制最高速度
                liner_speed = 0.8
            if liner_speed < 0.55:   # 对小车进行限制最低速度
                liner_speed = 0.55
